    ...
    const createTree = (ul, data) => {
        ...
        li.filter(d => !d.children)
            ...
            .html(d => {
                ...
                const minyear = d.minyear ? ` (${d.minyear})<sup>1</sup>` : '';  // Add minyear and superscript 1 if it exists
                const supr = d.supr === '1' ? '<sup>2</sup>' : '';  // Add superscript 2 if supr is 1 (those without cause specific data from WHO)
