...
// 1) Normal solid line generator (breaks on gaps)
const solidLine = d3.line()
  .defined(d => d[yVar] != null && !isNaN(d[yVar]))
  .x(d => xScale(d[xVar]))
  .y(d => yScale(d[yVar]));

// 2) Simple line generator for gap connectors (always connects)
const gapLine = d3.line()
  .x(d => xScale(d[xVar]))
  .y(d => yScale(d[yVar]));

// 3) Draw the solid lines
svg.selectAll(".line-path")
  .data(flattenedData)
  .enter()
  .append("path")
  .attr("class", "line-path")
  .attr("d", d => solidLine(d.dataPoints))
  .attr("fill", "none")
  .attr("stroke", d => d.color)
  .attr("stroke-width", 2.7)
  .attr("stroke-dasharray", d => d.dashArray);

// 4) For each series, find gap segments
flattenedData.forEach(series => {
  const gaps = [];
  const dp = series.dataPoints;
  for (let i = 0; i < dp.length - 1; i++) {
    const currDefined = dp[i][yVar] != null && !isNaN(dp[i][yVar]);
    const nextDefined = dp[i+1][yVar] != null && !isNaN(dp[i+1][yVar]);
    if (currDefined && !nextDefined) {
      // Search ahead for the next defined point
      for (let j = i + 1; j < dp.length; j++) {
        if (dp[j][yVar] != null && !isNaN(dp[j][yVar])) {
          // Found the next defined point after the gap
          gaps.push([dp[i], dp[j]]);
          break;
        }
      }
  ...
  // 5) Draw a dotted line for each gap
  svg.selectAll(null)
    .data(gaps)
    .enter()
    .append('path')
