...
use  iso3 year age sex pop if iso3!="" & year==2023 & sex == 3 using "$data/unwpp/population and deaths/estimates/data.dta", clear
collapse (sum)  pop , by(iso3)
save pop, replace
...
replace value = 0.000000000001 if value == 0
 
bys lid sex ageg (year): gen aapc = ///
    (ln(value[_N]) - ln(value[1])) / (year[_N] - year[1]) ///
    if _N == 2 & year[_N] > year[1] & value[1] > 0 & value[_N] > 0
	
	
	
bys lid sex ageg (year): gen projected2050 = ///
    exp(ln(value[_N]) + ///
    ((ln(value[_N]) - ln(value[1])) / (year[_N] - year[1])) * ///
    (2050 - year[_N])) ///
    if year[_N] != year[1] & value[1] > 0 & value[_N] > 0

bys lid sex ageg (year): gen x = (value[_N]/2)/projected2050


bys lid sex ageg (year): gen pct_decrease2050 = ///
    100 * (value[_N] - projected2050) / value[_N] ///
    if value[_N] > 0 & projected2050 < .

bys lid sex ageg (year): keep if _n==_N	
...
forval i = 1/3 {
_pctile aapc if ageg == "`y'" & pop > 5000 & pop < . & sex == `i', nq(3)
replace tercile = 1   if value<=r(r1) & sex==`i' & ageg=="`y'"
replace tercile = 2 if value> r(r1) & value<=r(r2) & sex==`i' & ageg=="`y'"
replace tercile = 3 if value> r(r2)  & sex==`i' & ageg=="`y'"
}
}

drop loc heading1 heading2 loc iso3 x iso3
compress
export delimited "$output_dir\main5data2" , replace
