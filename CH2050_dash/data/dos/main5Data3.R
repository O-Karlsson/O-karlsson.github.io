# Build main5data3.csv
#
# This starts from the R translation of CH2050_dash/data/dos/main5Data2.do and
# adds a second set of trend columns:
#   * aapc/projected2050/pct_decrease2050/tercile use the original trend window,
#     comparing the most recent value with the value about 10 years earlier.
#   * aapc2/projected20502/pct_decrease20502/tercile2 use the same most recent
#     value, but compare it with the value about 5 years earlier.
#
# The script keeps a few odd-looking details from the Stata code, including the
# final tercile assignment rule, because changing them would change the output.
...
trend_aapc <- function(value, year, lag_years) {
  # Compare the latest observation with the observation closest to lag_years
  # before it.  Exact years are available for mortality and height; math uses
  # survey years, so "closest" matches the original Stata approach.
  ord <- order(year)
  value <- value[ord]
  year <- year[ord]
  n <- length(value)
  ...
  prior <- seq_len(n - 1)
  base <- prior[which.min(abs(year[n] - year[prior] - lag_years))]

  if (year[n] > year[base] && value[base] > 0) {
    (log(value[n]) - log(value[base])) / (year[n] - year[base])
  } else {
    NA_real_
  }
}

trend_project_2050 <- function(value, year, lag_years) {
  # Project the latest value to 2050 using the annualized change over the chosen
  # lag window.
  ord <- order(year)
  value <- value[ord]
  year <- year[ord]
  ...
  if (year[n] > year[base] && value[base] > 0) {
    exp(log(value[n]) + ((log(value[n]) - log(value[base])) /
                           (year[n] - year[base])) * (2050 - year[n]))
  } else {
    NA_real_
  }
}
...
life_tables <- read_dta(path_clean("unwpp", "life tables", "estimates", "data.dta")) %>%
  ...
  filter(iso3 != "", age < 20, year %in% c(2013, 2018, 2023))
...
height_population <- read_dta(path_clean("unwpp", "population and deaths", "estimates", "data.dta")) %>%
  ...
  filter(age == 19, year %in% c(2009, 2014, 2019), iso3 != "")
...
temp2 <- height_population %>%
  ...
  filter(age == 19, year %in% c(2009, 2014, 2019)) %>%
...
# Keep the reference maximum anchored to the original 2009/2019 source years so
# the existing hgap value/aapc/projected columns remain comparable with
# main5data2.  The added 2014 rows are then placed on that same scale for the
# new 5-year trend columns.
height_gap <- temp2 %>%
  group_by(sex) %>%
  mutate(
    reference_height = max(mean_height[year %in% c(2009, 2019)], na.rm = TRUE),
    e = reference_height - mean_height
  ) %>%
  ungroup() %>%
  mutate(ageg = "hgap") %>%
...
# For each country/sex, keep the most recent math observation, the observation
# closest to ten years before it, and the observation closest to five years
# before it.  The 10-year rows preserve the original columns; the 5-year rows
# feed the new *2 trend columns.
temp <- temp %>%
  group_by(loc, sex) %>%
  mutate(
    maxyear = max(year, na.rm = TRUE),
    distance_from_maxyear = abs(maxyear - year - 10),
    distance_from_maxyear2 = abs(maxyear - year - 5)
  ) %>%
  arrange(distance_from_maxyear, .by_group = TRUE) %>%
  mutate(keep10 = row_number() == 1 | year == maxyear) %>%
  arrange(distance_from_maxyear2, .by_group = TRUE) %>%
  mutate(keep5 = row_number() == 1 | year == maxyear) %>%
  ungroup() %>%
  mutate(math_max10_source = if_else(keep10, e, NA_real_)) %>%
  group_by(loc, sex) %>%
  mutate(math_max10 = max(math_max10_source, na.rm = TRUE)) %>%
  ungroup() %>%
  filter(keep10 | keep5) %>%
  select(-distance_from_maxyear, -distance_from_maxyear2, -maxyear,
         -keep10, -keep5, -math_max10_source) %>%
  mutate(
    heading1 = "Countries",
    heading2 = region,
    ageg = "math"
...
# Stata runs "sum e" after keeping the 10-year and latest rows, then replaces
# e with r(max) - e.  Preserve that scale for the original columns even though
# this script also keeps the 5-year row.
math_max <- max(temp$math_max10, na.rm = TRUE)
temp <- temp %>%
  mutate(e = math_max - e) %>%
  select(-math_max10)
...
final_data <- bind_rows(temp, temp2) %>%
  ...
  mutate(
    aapc = trend_aapc(value, year, 10),
    projected2050 = trend_project_2050(value, year, 10),
    x = (value[n()] / 2) / projected2050,
    pct_decrease2050 = latest_pct_decrease(value, projected2050),
    aapc2 = trend_aapc(value, year, 5),
    projected20502 = trend_project_2050(value, year, 5),
    pct_decrease20502 = latest_pct_decrease(value, projected20502)
  ) %>%
  filter(row_number() == n()) %>%
  ungroup() %>%
...
# The new tercile2 column applies the same assignment rule as tercile, but uses
# the 5-year aapc2 cut points.
for (this_ageg in sort(unique(final_data$ageg))) {
  for (this_sex in 1:3) {
    cut_source <- final_data %>%
      filter(ageg == this_ageg, pop > 5000, !is.na(pop), sex == this_sex) %>%
    ...
    row_index <- final_data$ageg == this_ageg & final_data$sex == this_sex
    final_data$tercile2[row_index & final_data$value <= cuts[1]] <- 1L
    final_data$tercile2[row_index & final_data$value > cuts[1] &
                          final_data$value <= cuts[2]] <- 2L
    final_data$tercile2[row_index & final_data$value > cuts[2]] <- 3L
  }
}

final_data <- final_data %>%
  select(year, sex, value, ageg, lid, aapc, projected2050, pct_decrease2050,
         pop, tercile, aapc2, projected20502, pct_decrease20502, tercile2)

write_csv(final_data, path_out("main5data3.csv"), na = "")
